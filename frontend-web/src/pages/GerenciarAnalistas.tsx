...
const GerenciarAnalistas: React.FC = () => {
  ...
  const handleDeleteAnalista = (id: string): void => {
    if (window.confirm('Tem certeza que deseja excluir este analista?')) {
      alert(`Analista ${id} excluído com sucesso!`);
                ...
                <tr key={analista.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <button
                        onClick={() => handleManagePermissions(analista.id)}
                        className="text-purple-600 hover:text-purple-800 focus:outline-none focus:ring-2 focus:ring-purple-300 focus:ring-opacity-50 rounded p-1 transition-colors cursor-pointer"
                        title="Permissões"
                      >
